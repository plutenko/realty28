...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    const { data, error } = await supabase
      .from('crm_settings')
      .select('limits_enabled, limit_threshold, unclaimed_escalation_minutes')
      .eq('id', 1)
      .maybeSingle()
    ...
    return res.status(200).json(data || {
      ...
      limit_threshold: 10,
      unclaimed_escalation_minutes: 40,
    })
  }

  if (req.method === 'PATCH') {
    const { limits_enabled, limit_threshold, unclaimed_escalation_minutes } = req.body || {}
    ...
    if (Number.isInteger(unclaimed_escalation_minutes) && unclaimed_escalation_minutes > 0) {
      updates.unclaimed_escalation_minutes = unclaimed_escalation_minutes
    }
