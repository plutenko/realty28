...
/** Загрузка ВСЕХ квартир через complexes→buildings→units (обходит лимит 1000 строк Supabase) */
  ...
  try {
    const { data: complexes, error } = await supabase
      ...
      .select(`
        ...
        buildings (
          id, name, units_per_floor, units_per_entrance,
          handover_status, handover_quarter, handover_year,
          units (
            id, floor, number, position, entrance, rooms, area,
            layout_title, layout_image_url, finish_image_url,
            price, price_per_meter, status,
            span_columns, span_floors, is_commercial,
          ...
          flat.push({
            ...
            building: {
              ...
              complex: {
                ...
                developer: dev ? {
                  ...dev,
                  developer_managers: dev.developer_managers ?? [],
                } : null,
              },
            },
          })
