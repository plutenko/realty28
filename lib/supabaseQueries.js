...
/** Загрузка ВСЕХ квартир: сначала complexes+buildings, потом units отдельно по каждому дому */
export async function getAllUnitsViaComplexes(supabase) {
  if (!supabase) return { data: [], error: new Error('Supabase не настроен') }
  try {
    // 1) Загружаем структуру: complexes → buildings (без units)
    const { data: complexes, error: cErr } = await supabase
      .from('complexes')
      .select(`
        id, name, realtor_commission_type, realtor_commission_value,
        developer:developer_id (
          id, name,
          developer_managers ( id, name, phone, short_description, messenger, created_at )
        ),
        buildings ( id, name, floors, units_per_floor, units_per_entrance, handover_status, handover_quarter, handover_year )
      `)
      .order('name')

    if (cErr) return { data: [], error: cErr }

    // 2) Собираем все building_id и контекст
    ...
    for (const c of complexes ?? []) {
      ...
      for (const b of c.buildings ?? []) {
        buildingCtx.set(b.id, {
          building: b,
          complex: { id: c.id, name: c.name, realtor_commission_type: c.realtor_commission_type, realtor_commission_value: c.realtor_commission_value },
          developer: dev ? { ...dev, developer_managers: dev.developer_managers ?? [] } : null,
        })
      }
    }

    // 3) Загружаем units по всем building_id с пагинацией
    ...
    for (const bid of buildingIds) {
      ...
      while (true) {
        const { data: units, error: uErr } = await supabase
          .from('units')
          .select('id, floor, number, position, entrance, rooms, area, layout_title, layout_image_url, finish_image_url, price, price_per_meter, status, span_columns, span_floors, is_commercial, external_id, source_id')
          .eq('building_id', bid)
          .not('status', 'in', '("sold","booked","reserved")')
          .order('id')
          .range(from, from + PAGE - 1)

        if (uErr) break
        const ctx = buildingCtx.get(bid)
