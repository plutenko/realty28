  ...
  for (let floor = minFloor; floor <= maxFloor; floor += 1) {
    ...
    onFloor.forEach(({ item }, idx) => {
      ...
      units.push({
        ...
        // Амурстрой не раскрывает реальные номера; формируем синтетический
        // вида этаж*100 + позиция (501 = 5 этаж, 1 позиция). Это нужно для
        // рендера в /admin/units — UI прячет юниты без номера из импорта.
        number: String(floor * 100 + position),
        floor,
        position,
        entrance: 1,
