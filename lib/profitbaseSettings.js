...
/** Per-source override — set before sync, clear after */
let sourceOverride = null
export function setSourcePbOverride(settings) { sourceOverride = settings }
export async function clearSourcePbOverride() {
  sourceOverride = null
  // Сброс SSO кэша чтобы следующий источник получил свой JWT
  try { const { clearSsoCache } = await import('./profitbaseWidgetJwt.js'); clearSsoCache() } catch {}
}
...
export async function getProfitbaseSettings() {
  // 0) Per-source override (set during sync of a specific source)
  if (sourceOverride) {
