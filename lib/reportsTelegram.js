...
async function call(method, body) {
  ...
  // Ретраи ТОЛЬКО на явных API-ошибках (429/5xx). При network timeout/abort НЕ ретраим:
  // сообщение могло уже уйти в Telegram, ретрай = дубль (баг 2026-04-24 — Пт-notice
  // пришёл 2 раза).
  ...
  for (let i = 0; i < delays.length; i++) {
    ...
    try {
      ...
      console.warn(`[reports-tg] ${method} network error (not retrying, may have been sent):`, e?.message || e)
