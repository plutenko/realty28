...
import { formatMessengerLabel } from './leadsCore'
...
export function formatReactionTime(sec) {
 ...
 * Источник риелтору не показываем — только админам/менеджерам.
 */
export function formatLeadBlind(lead, source) {
  const lines = ['📥 <b>Новая заявка</b>', '']
  if (lead.name) lines.push(`Имя: ${escapeHtml(lead.name)}`)
  const mLabel = formatMessengerLabel(lead.messenger)
  if (mLabel) lines.push(`Мессенджер: ${escapeHtml(mLabel)}`)
...
/**
 * Карточка для победителя — с контактами. Источник риелтору не показываем.
 */
export function formatLeadForWinner(lead, source) {
  const lines = ['✅ <b>Вы взяли заявку</b>', '']
  if (lead.phone) {
    const mLabel = formatMessengerLabel(lead.messenger)
    lines.push(`📞 <code>${escapeHtml(lead.phone)}</code>${mLabel ? ` (${escapeHtml(mLabel)})` : ''}`)
  }
...
export function formatLeadTakenBy(lead, source, winnerName, reactionSec) {
  ...
  if (typeof reactionSec === 'number') lines.push(`за ${formatReactionTime(reactionSec)}`)
  lines.push('')
  if (lead.name) lines.push(`Имя: ${escapeHtml(lead.name)}`)
  return lines.join('\n')
}
...
export function formatLeadTakenForManager(lead, source, winnerName, reactionSec) {
  ...
  if (typeof reactionSec === 'number') lines.push(`Время реакции: ${formatReactionTime(reactionSec)}`)
  return lines.join('\n')
}
 ...
 * - Если crm_settings.limits_enabled=true, исключаем риелторов с
 *   new+in_work >= limit_threshold (чтобы не перегружать).
...
export async function broadcastLead(supabase, lead, source) {
  ...
  let active = (recipients || []).filter(r => r.is_active !== false && r.telegram_chat_id)
  if (active.length === 0) {
    console.warn('[leads-tg] no CRM recipients with Telegram — skipping rasśilka')
    return { sent: 0, total: 0 }
  }

  // Проверяем лимит активных лидов
  const { data: settings } = await supabase
    .from('crm_settings')
    .select('limits_enabled, limit_threshold')
    .eq('id', 1)
    .maybeSingle()
  const limitsOn = settings?.limits_enabled === true
  const threshold = settings?.limit_threshold || 10

  if (limitsOn) {
    const { data: active_loads } = await supabase
      .from('leads')
      .select('assigned_user_id')
      .in('status', ['new', 'in_work'])
      .in('assigned_user_id', active.map(r => r.id))
    const countsByUser = {}
    for (const l of active_loads || []) {
      countsByUser[l.assigned_user_id] = (countsByUser[l.assigned_user_id] || 0) + 1
    }
    const before = active.length
    active = active.filter(r => (countsByUser[r.id] || 0) < threshold)
    if (active.length < before) {
      console.log(`[leads-tg] skipping ${before - active.length} recipients over limit`)
    }
    if (active.length === 0) {
      console.warn('[leads-tg] all CRM recipients at limit — no rasśilka (escalation-cron поймает)')
      return { sent: 0, total: 0, allAtLimit: true }
    }
  }
 ...
 * Также эдитим эскалационные сообщения «никто не взял» руководителям.
 */
export async function editOtherRecipientsAfterTake(supabase, lead, source, winnerUserId, winnerName, reactionSec) {
  ...
  // Эдитим эскалационные уведомления «никто не взял» — теперь известно,
  // кто взял, чтобы у руководителя в чате не было путаницы.
  try {
    ...
    const resolvedText =
      `✅ <b>Лид взят: ${escapeHtml(winnerName)}</b>\n` +
      `за ${formatReactionTime(reactionSec)}\n\n` +
    ...
    for (const ev of escEvents || []) {
      ...
      for (const sent of list) {
        ...
        try {
          await editTelegramMessage(sent.chat_id, sent.message_id, resolvedText)
        } catch (e) {
          console.warn('[leads-tg] edit escalation failed', sent.chat_id, sent.message_id, e?.message || e)
        }
      }
    }
  } catch (e) {
    console.warn('[leads-tg] escalation edit lookup failed', e?.message || e)
  }
}
