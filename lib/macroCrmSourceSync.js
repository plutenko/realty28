...
export async function collectUnitsFromMacroCrmSource(source) {
  ...
  // MacroCRM API не отдаёт position. Для каждой пары (этаж, подъезд) считаем
  // position = number - minNumberOnFloor + 1. Внутренние пропуски номеров (62, 63 между
  // 61 и 64) становятся пустыми клетками как у застройщика. Edge-пропуски (если первый
  // номер этажа отсутствует) не показываются — их нужно добавить вручную в админке.
  ...
  for (const arr of groups.values()) {
    ...
    for (const u of arr) {
      ...
      u.position = Number.isFinite(n) ? n - minNum + 1 : null
    }
  }
