...
export async function collectUnitsFromMacroCrmSource(source) {
  ...
  // MacroCRM API не отдаёт position. Вычисляем относительно полного этажа:
  //   1. unitsPerFloor = максимальный span (max - min + 1) по этажам.
  //   2. pivot = самый полный этаж (span == unitsPerFloor или максимальный доступный).
  //   3. expectedFirst(F) = pivotFirst + unitsPerFloor * (F - pivotFloor).
  //   4. position = number - expectedFirst(F) + 1.
  // Это корректно показывает пропуски на КРАЯХ этажа (напр. floor 20 без #274 — gap слева),
  // а не только внутри ряда.
  const groups = new Map()
  for (const u of units) {
    const key = `${u.entrance ?? 0}`
    if (!groups.has(key)) groups.set(key, new Map())
    const byFloor = groups.get(key)
    const f = u.floor ?? 0
    if (!byFloor.has(f)) byFloor.set(f, [])
    byFloor.get(f).push(u)
  }
  for (const byFloor of groups.values()) {
    let bestSpan = 0
    let pivot = null // { floor, firstNum }
    for (const [f, arr] of byFloor) {
      ...
      if (span > bestSpan || (span === bestSpan && arr.length > (pivot?.count ?? 0))) {
        bestSpan = span
        pivot = { floor: f, firstNum: minN, count: arr.length }
      }
    }
    if (!pivot || bestSpan < 1) continue
    for (const [f, arr] of byFloor) {
      const expectedFirst = pivot.firstNum + bestSpan * (f - pivot.floor)
      for (const u of arr) {
        const n = Number(u.number)
        if (!Number.isFinite(n)) { u.position = null; continue }
        const p = n - expectedFirst + 1
        u.position = p >= 1 ? p : null
      }
    }
  }
