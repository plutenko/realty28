...
function mapObjectToUnit(obj, sourceId, buildingId, { skipImages = false } = {}) {
  const e = obj?.estate || {}
  ...
  const layoutUrl = skipImages ? null : pickPlanUrl(obj?.plans)
 ...
 */
export async function collectUnitsFromMacroCrmSource(source, { skipImages = false } = {}) {
  ...
  const units = objects.map((o) => mapObjectToUnit(o, source.id, buildingId, { skipImages }))
  ...
  //   3. позиция ГЛОБАЛЬНАЯ по всей сетке: offset подъезда = сумма ширин предыдущих.
  //      Шахматка ищет по позиции без учёта entrance, поэтому позиции должны быть
  //      уникальны по всему дому (иначе квартиры разных подъездов перекрывают друг друга).
  const groupsByEntrance = new Map()
  for (const u of units) {
    const key = u.entrance && u.entrance > 0 ? Number(u.entrance) : 1
  ...
  const entranceKeys = [...groupsByEntrance.keys()].sort((a, b) => a - b)
  const widthByEntrance = new Map()
  const floorOffsetByEntrance = new Map() // e => Map<floor, offsetWithinEntrance>
  for (const e of entranceKeys) {
    const byFloor = groupsByEntrance.get(e)
    ...
    for (const f of floors) {
      ...
      const off = gap > 0 ? gap : 0
      offsetByFloor.set(f, off)
      const width = (maxN - minN + 1) + off
      if (width > maxWidth) maxWidth = width
      prevMax = maxN
    }
    widthByEntrance.set(e, maxWidth || 1)
    floorOffsetByEntrance.set(e, offsetByFloor)
  }
  const entranceOffsets = new Map()
  let cumulative = 0
  for (const e of entranceKeys) {
    entranceOffsets.set(e, cumulative)
    cumulative += widthByEntrance.get(e) || 0
  }
  for (const e of entranceKeys) {
    const byFloor = groupsByEntrance.get(e)
    const entOffset = entranceOffsets.get(e) || 0
    const offsetByFloor = floorOffsetByEntrance.get(e)
    for (const [f, arr] of byFloor) {
      const nums = arr.map((u) => Number(u.number)).filter((n) => Number.isFinite(n))
      if (!nums.length) continue
      const minN = Math.min(...nums)
      const floorOff = offsetByFloor.get(f) || 0
      for (const u of arr) {
        const n = Number(u.number)
        u.position = Number.isFinite(n)
          ? entOffset + (n - minN + 1) + floorOff
          : null
      }
    }
  }

  const unitsPerEntrance = entranceKeys.map((e) => widthByEntrance.get(e) || 0)
  const unitsPerFloor = unitsPerEntrance.reduce((a, b) => a + b, 0)
  ...
  return {
    ...
    meta: {
      ...
      unitsPerEntrance: unitsPerEntrance.length ? unitsPerEntrance : null,
      entrancesCount: entranceKeys.length,
      count: units.length,
