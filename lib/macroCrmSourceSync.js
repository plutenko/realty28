...
export async function collectUnitsFromMacroCrmSource(source) {
  ...
  // MacroCRM API не отдаёт position — присваиваем 1..N, группируя по (этаж, подъезд)
  // и сортируя по номеру квартиры. Иначе админка считает position как number % units_per_floor,
  // что даёт пропуски при непоследовательной нумерации между секциями (напр. 59,60,61,64,65).
  const groups = new Map()
  for (const u of units) {
    const key = `${u.floor ?? 0}|${u.entrance ?? 0}`
    if (!groups.has(key)) groups.set(key, [])
    groups.get(key).push(u)
  }
  for (const arr of groups.values()) {
    arr.sort((a, b) => {
      const an = Number(a.number); const bn = Number(b.number)
      if (Number.isFinite(an) && Number.isFinite(bn)) return an - bn
      return 0
    })
    arr.forEach((u, i) => { u.position = i + 1 })
  }

  // meta: этажей и макс. квартир на этаже
  let floorsCount = 0
