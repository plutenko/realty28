...
export async function upsertImportedUnits(supabase, units) {
  ...
  if (normalized.length > 0) {
    ...
    // Разделяем по наличию валидного номера квартиры.
    // Уникальный ключ в БД — (building_id, number), поэтому для юнитов с номером
    // всегда используем его как conflict-key — иначе INSERT нового external_id
    // на уже занятый (building_id, number) падает с ошибкой уникальности.
    ...
    if (withNumber.length > 0) {
      // Дедуплицируем по (building_id, number) — оставляем последнее вхождение
    ...
    // Юниты без номера (коммерческие, паркинг и т.п.) — upsert по external_id
