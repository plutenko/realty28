...
/**
 * Публичный экспорт одного листа по gid — без OAuth.
 * Работает если файл открыт «Просматривать могут все по ссылке».
 * Google возвращает xlsx только с нужным листом.
 */
async function downloadSheetByGidPublic(spreadsheetId, gid) {
  const url = `https://docs.google.com/spreadsheets/d/${spreadsheetId}/export?format=xlsx&gid=${gid}`
  const resp = await fetch(url, { redirect: 'follow' })
  if (!resp.ok) throw new Error(`HTTP ${resp.status}`)
  const ct = resp.headers.get('content-type') || ''
  // Если Google вернул HTML (страница логина) — файл не публичный
  if (ct.includes('text/html')) throw new Error('Not public: Google returned HTML')
