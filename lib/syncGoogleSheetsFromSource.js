  ...
  try {
    ...
    if (isSpreadsheetMetadataUnsupportedError(err)) {
      ...
      if (names.length >= 2) {
        ...
        const sheetList = names
          .map((n, i) => `  sync_sheet=${i} → «${n}»`)
          .join('\n')
        throw new Error(
          `Внешний Excel, ${names.length} листов — #gid не работает для таких файлов. Добавьте в URL параметр &sync_sheet= с номером или именем нужного листа:\n${sheetList}`
        )
      }
    }
 ...
 */
export async function resolveGoogleAuthForSheetsSync(supabase) {
  const row = await fetchGoogleSheetsOAuthRow(supabase)
...
export function isGoogleSheetsSodruzhestvoParserType(source) {
  const pt = String(source?.parser_type || '').trim().toLowerCase()
  if (!pt || pt === 'default' || pt === 'sodruzhestvo' || pt === 'содружество')
    return true
  return false
}
...
/**
 * Маркетинговый этаж Содружество: 201 → 2, 301 → 3 (сотни в номере).
 * После экспорта xlsx колонка A часто «уезжает»; парсер ещё может ошибочно дать span_floors=2.
 * Двухуровневая 601 (охват 5–6): в номере «6», нижний этаж в парсере 5 — оставляем min.
 *
 * @param {Record<string, unknown>} row
 */
function sodruzhestvoFloorFromRow(row) {
  const inferred = inferFloorFromFlatNumber(row.number)
  const rf = Number(row.floor)
  const sf =
    typeof row.span_floors === 'number' ? row.span_floors : 1

  if (inferred == null) {
    return Number.isFinite(rf) ? rf : row.floor
  }
  if (sf > 1 && Number.isFinite(rf) && rf < inferred) {
    return rf
  }
  return inferred
}
 ...
 */
export async function downloadSpreadsheetXlsxBuffer(spreadsheetId, auth) {
...
/**
 * Публичный экспорт одного листа по gid — без OAuth, через Node.js https.
 * Работает если файл открыт «Просматривать могут все по ссылке».
 * Google возвращает xlsx только с нужным листом.
 */
function downloadSheetByGidPublic(spreadsheetId, gid) {
  return new Promise((resolve, reject) => {
    const startUrl = `https://docs.google.com/spreadsheets/d/${spreadsheetId}/export?format=xlsx&gid=${gid}`

    function doGet(rawUrl, redirects) {
      if (redirects > 5) { reject(new Error('Too many redirects')); return }
      let parsed
      try { parsed = new URL(rawUrl) } catch { reject(new Error('Invalid URL')); return }
      const req = https.get(
        { hostname: parsed.hostname, path: parsed.pathname + parsed.search, headers: { 'User-Agent': 'Node' }, timeout: 20000 },
        (resp) => {
          ...
          if (statusCode !== 200) { resp.resume(); reject(new Error(`HTTP ${statusCode}`)); return }
          const ct = headers['content-type'] || ''
          if (ct.includes('text/html')) { resp.resume(); reject(new Error('Not public: HTML response')); return }
          const chunks = []
...
export async function syncGoogleSheetsFromSource(supabase, source) {
  ...
  try {
    ...
    const sheetHint =
      (source.sheet_name ? String(source.sheet_name).trim() : '') ||
      extractSheetSyncHint(source.url) ||
      ''
    ...
    if (gid) {
      // Шаг 1: публичный экспорт конкретного листа (без OAuth).
      // Google экспортирует xlsx только с нужной вкладкой → первый лист и есть нужный.
      let publicOk = false
      try {
        buffer = await downloadSheetByGidPublic(spreadsheetId, gid)
        const wb = XLSX.read(buffer, { type: 'buffer', bookSheets: false })
        const names = wb.SheetNames || []
        if (names.length > 0) {
          sheetName = names[0]
        ...
        // файл не публичный или другая ошибка — переходим к OAuth
      }

      if (!publicOk) {
        // Шаг 2: OAuth + Drive API + Sheets API для разрешения gid
        const { auth: googleAuth, usedOAuth: oauth } =
          await resolveGoogleAuthForSheetsSync(supabase)
        usedOAuth = oauth
        const { buffer: oauthBuffer } = await downloadSpreadsheetXlsxBuffer(spreadsheetId, googleAuth)
        buffer = oauthBuffer
        sheetName = await resolveSheetTabNameWithFallback(
          buffer, spreadsheetId, gid, googleAuth, undefined
        )
      }
    } else {
      // Нет gid → OAuth обязателен
      const { auth: googleAuth, usedOAuth: oauth } =
        await resolveGoogleAuthForSheetsSync(supabase)
      usedOAuth = oauth
      const { buffer: oauthBuffer } = await downloadSpreadsheetXlsxBuffer(spreadsheetId, googleAuth)
      ...
      if (sheetHint) {
        ...
        if (!picked) {
          return await fail(
            `Лист «${sheetHint}» не найден. Доступные листы: ${names.length ? names.map((n, i) => `${i}: «${n}»`).join(', ') : '(нет листов)'}.`
          )
        }
        sheetName = picked
      } else {
        ...
        const names = wb.SheetNames || []
        if (!names.length) {
          return await fail('Файл не содержит листов.')
        }
        if (names.length === 1) {
          sheetName = names[0]
        } else {
          return await fail(
            `Файл содержит ${names.length} листов. Выберите нужный лист в настройках источника.`
          )
        }
      }
    }
    ...
    const statusCounts = {}
    for (const u of parsed) { statusCounts[u.status] = (statusCounts[u.status] || 0) + 1 }
    console.log(`[syncGoogleSheets] source=${source.id} sheet="${sheetName}" parsed=${parsed.length} statuses:`, statusCounts)

    const commercialDebug = parsed.filter(u => u.is_commercial).map(u => ({
      number: u.number, area: u.area, price: u.price, ppm: u.price_per_meter
    }))
    ...
    const apartments = parsed
      ...
      .map((row) => {
        const floor = sodruzhestvoSheet
          ? sodruzhestvoFloorFromRow(row)
          : row.floor
        const price = row.price != null ? Number(row.price) : 0
        const area = row.area != null ? Number(row.area) : 0
        // For Sodruzhestvo: calculate ppm from price/area (sheet row B is unreliable)
        const ppm = sodruzhestvoSheet
    ...
    const commercial = parsed
      ...
      .map((row) => {
        ...
        const priceC = row.price != null ? Number(row.price) : 0
        const areaC = row.area != null ? Number(row.area) : 0
        const ppmC = sodruzhestvoSheet
          ? (areaC > 0 && priceC > 0 ? Math.round(priceC / areaC) : 0)
          : (row.price_per_meter != null ? Number(row.price_per_meter) : 0)
        return {
          source_id: source.id,
          building_id: source.building_id,
    ...
    const { count } = await upsertImportedUnits(supabase, units, { conflictKey: 'building_id,number' })
    ...
    // Include status debug info in response
    const finalStatusCounts = {}
    for (const u of units) { finalStatusCounts[u.status] = (finalStatusCounts[u.status] || 0) + 1 }
    // Debug: sample parsed raw data with fill info
    const parsedHighFloor = parsed.filter(u => u.floor >= 16 && !u.is_commercial).slice(0, 3).map(u => ({
      number: u.number, floor: u.floor, status: u.status, _fillDebug: u._fillDebug
    }))
    return { ok: true, count, debug: { statusCounts: finalStatusCounts, total: units.length, parsedHighFloor, commercialDebug } }
  } catch (e) {
