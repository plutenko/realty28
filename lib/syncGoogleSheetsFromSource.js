...
export async function syncGoogleSheetsFromSource(supabase, source) {
  ...
  try {
    ...
    let sheetName
    if (gid) {
      // Приоритет — gid из URL: стабильный числовой ID, не меняется при переименовании листа.
      // sheet_name используется как запасной вариант если gid не сработал (напр. внешний xlsx).
