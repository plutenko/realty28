...
export async function collectUnitsFromPikSource(source) {
  ...
  // Максимум квартир в секции на любом этаже = ширина секции в шахматке
  const maxPerSection = {}
  ...
  const unitsPerEntrance = sections.map(s => maxPerSection[s])
  ...
  for (const [key, arr] of bySecFloor) {
    ...
    let offset = 0
    for (let i = 0; i < secIdx; i++) offset += unitsPerEntrance[i]
