...
export async function collectUnitsFromPikSource(source, existingUnitsPerEntrance = null) {
  ...
  // Загружаем реальные номера квартир с /flat/{id} параллельно (батчами по 5)
  ...
  for (let i = 0; i < flats.length; i += BATCH) {
    ...
    await Promise.all(batch.map(async f => {
      try {
        ...
        const num = j?.props?.pageProps?.initialState?.flat?.flatService?.flat?.number
        if (num != null) realNumberById.set(f.id, String(num))
      } catch {}
    }))
  }
