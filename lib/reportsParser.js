...
export function classifyMessage(text, settings) {
  if (!text) return 'none'
  const first = firstLine(text).trim().toLowerCase()
  ...
  for (const word of Object.keys(absence)) {
    if (first.startsWith(word.toLowerCase())) return 'absence'
  }
...
export function parseAbsence(text, settings, now = new Date()) {
  ...
  for (const word of Object.keys(absence)) {
    if (low.startsWith(word.toLowerCase())) {
      matched = { word, type: absence[word] }
      break
    }
  }
  if (!matched) return { ok: false, errors: [{ type: 'no_date', value: first }] }

  // Откидываем слово-маркер + возможное "за"/"от"/"с"
  let rest = first.slice(matched.word.length).trim().replace(/^(?:за|от)\s+/i, '').trim()

  // Если ничего не осталось — подставляем дату по времени написания:
  // >= 12:00 текущего дня → сегодня (окно сегодняшнего дня открыто)
  // < 09:30 текущего дня → вчера (ещё идёт окно приёма вчерашнего)
  // 09:30 – 12:00 → окно ни туда ни сюда, просим указать явно
  if (!rest) {
    const def = computeDefaultAbsenceDate(nowLocal, settings)
    if (!def) {
      return { ok: false, errors: [{ type: 'no_date', value: first }] }
    }
    return {
      ok: true,
  ...
  // Пробуем парсить дату или диапазон теми же функциями что и для отчёта
  const headerLine = `${settings.report_marker_words?.[0] || 'Отчёт'} ${rest}`
...
export function parseReport(text, settings, now = new Date()) {
  ...
  // ---- 4. Экстра-строки (то что не распознано как метрика и не пустое)
  for (let i = 1; i < lines.length; i++) {
    if (usedLines.has(i)) continue
    ...
    if (m) {
      result.extra[m[1].trim()] = m[2].trim()
    } else {
