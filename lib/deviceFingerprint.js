...
export function computeDeviceHash({ userAgent = '', clientHints = {} }) {
  ...
  // Намеренно НЕ включаем браузер и IP — чтобы Chrome/Firefox на одном компе
  // и разные сети (wifi/мобильный) давали один и тот же хэш.
  const raw = `${os}|${platform}|${screen}|${timezone}`
