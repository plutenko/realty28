...
const ssoCacheByAccount = new Map()

/** Сброс кэша SSO (вызывается при переключении между per-source настройками) */
export function clearSsoCache() { ssoCacheByAccount.clear() }

function getSsoCache(accountId) {
  return ssoCacheByAccount.get(accountId) || { token: '', expiresAtMs: 0 }
}

function setSsoCache(accountId, token, expiresAtMs) {
  ssoCacheByAccount.set(accountId, { token, expiresAtMs })
}
...
async function fetchSsoSiteWidgetToken() {
  ...
  let refNorm = referer.replace(/\/$/, '')
  if (refNorm && !/^https?:\/\//i.test(refNorm)) {
    refNorm = `https://${refNorm}`
  }
  let res
  try {
    res = await fetch(url, {
  ...
  setSsoCache(accountId, json.access_token, Date.now() + expiresIn * 1000 - SSO_HEADROOM_MS)
...
export async function getProfitbaseWidgetJwt() {
  ...
  const settings = await getProfitbaseSettings()
  const acctId = settings.accountId || tenantId()
  const hasAuto = acctId && (settings.siteWidgetReferer || siteWidgetReferer())
  if (hasAuto) {
    const cached = getSsoCache(acctId)
    if (cached.token && Date.now() < cached.expiresAtMs) {
      return cached.token
    }
