 ...
 * У Profitbase в smallGrid/board поле этажа строки иногда считается иначе (техэтажи,
 * парковка), из‑за этого 201 оказывается не на 2-м этаже в нашей шахматке.
 * @param {unknown} num
 * @returns {number | null}
 */
...
export async function collectUnitsFromProfitbaseSource(source) {
  ...
  if (!cells.length) {
    return {
      units: [],
      error: `Не удалось получить структуру board/smallGrid для house ${houseId}. ${
        boardRes?.error || ''
      }`.trim(),
    }
  }
  ...
  if (!units.length) {
    return { units: [], error: 'smallGrid получен, но не нашли ни одной квартиры propertyId.' }
  }
