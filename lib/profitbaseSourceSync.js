...
function mapProfitbasePropertyDetail(json, externalId, source, { skipImages = false } = {}) {
  let d =
    json?.data?.property ??
    json?.data ??
    json?.property ??
  ...
  const layoutUrl = skipImages
    ? null
    : (d.preset || (Array.isArray(d.planImages) && d.planImages[0]?.source) || null)
...
/**
 * /floor?houseId=… возвращает массив этажей: каждый объект содержит `images`
 * (source / big / large / preview — PNG плана этажа) и `areas[]` с propertyId
 * квартир на этом этаже. SVG с подсветкой нет — подсветка рисуется фронтом из
 * координат areas. Сохраняем общий план этажа в images.building_floor_level_plan.
 */
function parseFloorPlansByPropertyId(text) {
  ...
  const map = new Map()
  for (const fl of arr) {
    // source = оригинал PNG без пережатия; large/big — уменьшённые thumbnail'ы
    // (2000/10000 px), которые теряют резкость подписей комнат. Берём source.
    const img =
      fl?.images?.source || fl?.images?.large || fl?.images?.big || null
    if (!img) continue
    const areas = Array.isArray(fl?.areas) ? fl.areas : []
    for (const a of areas) {
      const pid = a?.propertyId
      if (pid == null) continue
      const key = String(pid)
      if (!map.has(key)) map.set(key, String(img))
    }
  }
  return map
}
 ...
 */
export async function collectUnitsFromProfitbaseSource(source, { skipImages = false } = {}) {
  ...
  if (list.ok && list.text?.trim()) {
    try {
      ...
      for (const item of rows) {
        ...
        if (!ext) continue
        const u = mapProfitbasePropertyDetail({ data: item }, ext, source, { skipImages })
    ...
    const batch = await Promise.all(
      slice.map(async (propertyId) => {
        ...
        try {
          ...
          return { id: propertyId, unit: mapProfitbasePropertyDetail(pj, propertyId, source, { skipImages }) }
        } catch {
          return null
        }
      })
  ...
  // 4) Поэтажные планы (общие, один на этаж). /floor отдаёт PNG + координаты
  //    зон квартир; подсветка конкретной квартиры рисуется фронтом из areas,
  //    персонального плана per-квартиру у Profitbase нет — кладём общий план
  //    в images.building_floor_level_plan (как MacroCRM/Amurstroy).
  // skipImages=true — пропускаем floors-фетч, существующие планы остаются в БД.
  let planByPropertyId = new Map()
  if (!skipImages) {
    const floorsRes = await fetchProfitbaseCrmFloorsBody(houseId)
    if (floorsRes.ok && floorsRes.text) {
      planByPropertyId = parseFloorPlansByPropertyId(floorsRes.text)
    }
  }

  // 5) Create units only for occupied cells. Этаж берём из board grid (приоритет),
  //    inferFloorFromFlatNumber используется только если grid-этаж отсутствует.
  ...
  for (const c of cells) {
    ...
    // Board grid floor is authoritative; infer only as fallback
    const floor = Number.isFinite(floorGrid) && floorGrid > 0
      ? floorGrid
      : (inferFloorFromFlatNumber(detail.number) ?? 0)
    ...
    // Первый встреченный план на этаж — пишем в общий список для images.
    if (Number.isFinite(floor) && floor > 0 && !floorPlansByLevel.has(floor)) {
      const planUrl = planByPropertyId.get(String(c.propertyId))
      if (planUrl) floorPlansByLevel.set(floor, planUrl)
    }
  }
  ...
  const floorPlans = [...floorPlansByLevel.entries()].map(([floor_level, url]) => ({
    floor_level,
    url,
  }))
