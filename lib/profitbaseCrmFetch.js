...
import {
  ...
  fetchProfitbaseFloorsWithSiteWidgetBearer,
  fetchProfitbaseFloorsWithV4,
...
export async function fetchProfitbaseCrmFloorsBody(houseId) {
  const id = String(houseId || '').trim()
  if (!id) return { ok: false, error: 'houseId required' }

  const jwt = await getProfitbaseWidgetJwt()
  if (jwt) {
    const sw = await fetchProfitbaseFloorsWithSiteWidgetBearer(id, jwt)
    if (sw.ok) return { ok: true, text: sw.text }
  }

  if (profitbaseV4Configured()) {
    const v4 = await fetchProfitbaseFloorsWithV4(id)
    if (v4.ok) return { ok: true, text: v4.text }
  }

  return { ok: false, error: 'floor недоступен (JWT / v4)' }
}
