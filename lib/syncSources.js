...
import { collectUnitsFromLazurnyyberegeSource } from './lazurnyyberegeSourceSync'
...
export async function syncSource(source, providedSupabase = null, options = {}) {
  ...
  if (typeLc === 'fsk') {
    const { units, error, meta } = await collectUnitsFromFskSource(source, { supabase, skipImages })
    if (error) throw new Error(error)
    if (meta?.unitsPerFloor && Number(meta.unitsPerFloor) > 0) {
  ...
  if (typeLc === 'lazurnyy_bereg') {
    const { units, error, meta } = await collectUnitsFromLazurnyyberegeSource(source)
    if (error) throw new Error(error)
    const upd = {}
    if (meta?.unitsPerFloor && Number(meta.unitsPerFloor) > 0) {
      upd.units_per_floor = Number(meta.unitsPerFloor)
  ...
  if (typeLc === 'amurstroy') {
    ...
    const { count } = await upsertImportedUnits(supabase, units)
    // Поэтажные планы: upsert в images (entity_type=building_floor_level_plan).
    // Только на этажи, где ещё нет записи (не перетираем ручные загрузки).
    if (Array.isArray(meta?.floorPlans) && meta.floorPlans.length > 0 && building?.id) {
      const { data: existing } = await supabase
        .from('images')
        .select('floor_level')
        .eq('entity_type', 'building_floor_level_plan')
        .eq('entity_id', building.id)
      const have = new Set((existing ?? []).map((r) => r.floor_level))
      const rows = meta.floorPlans
        .filter((fp) => !have.has(fp.floor_level))
        .map((fp) => ({
          entity_type: 'building_floor_level_plan',
      ...
      if (rows.length > 0) {
        ...
        // schema cache errors — поле может быть не создано в старых инстансах
        if (imgErr && !/floor_level|column|schema cache/i.test(String(imgErr.message || ''))) {
          throw imgErr
        }
      }
    }
  ...
  if (isMacroCrmType) {
    const { units, error, meta } = await collectUnitsFromMacroCrmSource(source, { skipImages })
    if (error) throw new Error(error)
    if (meta?.unitsPerFloor && Number(meta.unitsPerFloor) > 0) {
      const upd = { units_per_floor: Number(meta.unitsPerFloor) }
      if (meta?.floorsCount && Number(meta.floorsCount) > 0) upd.floors = Number(meta.floorsCount)
    ...
    const { count } = await upsertImportedUnits(supabase, units)
    // Поэтажные планы: insert в images (building_floor_level_plan), только на этажи
    // где ещё нет ручно загруженной картинки — не перетираем руками загруженное.
    if (Array.isArray(meta?.floorPlans) && meta.floorPlans.length > 0 && building?.id) {
      const { data: existing } = await supabase
        .from('images')
        .select('floor_level')
  ...
  if (isProfitbaseType) {
    ...
    let syncResult
    try {
      syncResult = await collectUnitsFromProfitbaseSource(source, { skipImages })
    } finally {
      await clearSourcePbOverride()
    }
    ...
    // Поэтажные планы (общие на этаж): upsert в images, только на этажи без
    // уже загруженной ручной картинки — чтобы не перетирать ручные планы.
    if (Array.isArray(meta?.floorPlans) && meta.floorPlans.length > 0 && building?.id) {
      const { data: existing } = await supabase
        .from('images')
        .select('floor_level')
...
export async function syncAllSources(options = {}) {
  ...
  for (const source of sources ?? []) {
    try {
      const one = await syncSource(source, supabase, options)
