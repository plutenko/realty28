...
export async function syncSource(source, providedSupabase = null) {
  ...
  if (typeLc === 'pik') {
    ...
    const hasExistingEntrance = Array.isArray(building?.units_per_entrance) && building.units_per_entrance.length > 0
    if (meta?.unitsPerFloor && Number(meta.unitsPerFloor) > 0) {
      ...
      if (!hasExistingEntrance && Array.isArray(meta?.unitsPerEntrance) && meta.unitsPerEntrance.length > 0) {
