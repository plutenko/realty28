...
export async function syncSource(source, providedSupabase = null) {
  ...
  if (typeLc === 'pik') {
    ...
    // Заполняем building поля только если админ ещё не задал — не перезатираем вручную выставленные значения.
    const upd = {}
    if (!building?.units_per_floor && meta?.unitsPerFloor && Number(meta.unitsPerFloor) > 0) {
      upd.units_per_floor = Number(meta.unitsPerFloor)
    }
    if (!building?.floors && meta?.floorsCount && Number(meta.floorsCount) > 0) {
