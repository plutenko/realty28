...
- **Пример:** литер **14А** (и прочие литеры Содружества) — отдельная вкладка в одной Google-таблице; этаж квартиры в БД = **число в колонке A** у блока этажа в файле (напр. 201 на 2-м этаже, если в A стоит `2`). Это **не** синк Profitbase: эвристика «этаж из номера квартиры» в `profitbaseSourceSync.js` сюда не относится.
